...
function fromMM(val) { fromUnit("mm", val); }
function fromCM(val) { fromUnit("cm", val); }
function fromInch(val) { fromUnit("inch", val); }
function fromFeet(val) { fromUnit("feet", val); }
function fromPixel(val) { fromUnit("pixel", val); }
...
function editPixelFactor() {
  ...
  if (!isNaN(parsed) && parsed > 0) {
    inchToPixel = parsed;
    document.getElementById("pxLabel").innerText = `current 1 inch = ${parsed} pixel`;
